...
// It's been really difficult to record from the separate audio and video streams wffmpeg -y -i vid.mp4 -f mp4 -i aud.mp4 -f mp4 test.mp4ith the result in sync, this gives the best result so far, with the
// annoyance of continuous warnings during the first pass. First run : =
// ffmpeg -y -f alaw -thread_queue_size 2048 -use_wallclock_as_timestamps 1 -i http://localhost:8081/h/stream?rsuuid=cam1-stream1a -f hevc -thread_queue_size 2048 -use_wallclock_as_timestamps 1 -i http://localhost:8081/h/stream?rsuuid=cam1-stream1 -f mp4 -map 1:v -c:v copy vid.mp4 -f mp4 -map 0:a -c:a aac aud.mp4
// To produce separate audio and video mp4 files with timestamps for both set from the wallclock, then combine them with : =
// ffmpeg -y -i vid.mp4 -f mp4 -i aud.mp4 -f mp4 test.mp4
