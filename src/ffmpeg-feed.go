		...
		for _, stream := range camera.Streams {
			go func(camera Camera, stream StreamC) {
				for {
					...
					cmdStr := fmt.Sprintf("/usr/bin/ffmpeg -loglevel warning -hide_banner %s-fflags nobuffer -rtsp_transport %s -i  %s -c:v copy %s  -f hevc -preset ultrafast -tune zero_latency %s -vn -c:a pcm_s16le -b:a 64K -ar 8000 -af atempo=1.02 -f s16be -preset ultrafast -tune zero_latency http://localhost:8081/live/stream?suuid=stream1a", stimeout, rtspTransport, netcamUri, audio, stream.MediaServerInputUri)
